...
customElements.define(
  ...
  class Footer extends HTMLElement {
    constructor() {
      ...
      let columns = [];
      this.querySelectorAll('ul').forEach((colum) =>
        columns.push(this.makeColumn(colum))
      );

      if (columns.length > 3) {
        console.warn(
          'Guidelines disallows more than 3 sections, please consider moving more links to the first columns.'
        );
      ...
      if (social) this.querySelector('slot[name=social]').appendChild(social);
      else this.querySelector('slot[name=social]').remove();
      columns.forEach((col) =>
        this.querySelector('slot[name=content]').appendChild(col)
      );
    }
      ...
      if (!el) {
        ...
        el.appendChild(
          document.createTextNode(
            `© ${new Date().getFullYear()} AXA All Rights Reserved`
          )
        );
      }
