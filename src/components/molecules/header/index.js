...
customElements.define(
  ...
  class HeaderMenu extends HTMLElement {
    ...
    constructor() {
      ...
      if (originalStructure) {
        ...
        firstLevel.forEach(function (el) {
          ...
          if (el.parentNode !== originalStructure) return;
      ...
      this.insertAdjacentHTML(
        ...
        `
            <nav class=m-header>
               ...
               ${
                 originalStructure
                   ? ' <axa-icon name=menu icon=menu aria-haspopup=true></axa-icon>'
                   : ''
               }
            </nav>
            `
      ...
      if (originalStructure) {
        this.querySelector('slot[name=structure]').appendChild(
          originalStructure
        );
        this.setupIconMenu();
      ...
      if (cta) this.querySelector('slot[name=call-to-action]').appendChild(cta);

      // demo mode : prevent every click
      //this.addEventListener('click', e => e.preventDefault())
      if (this.isSearchable) this.setupSearch();
    ...
    setupSearch() {
      ...
      if (!svgEl)
        // retry a bit later, when the DOM of the button is really ready
    ...
    setupIconMenu() {
      ...
      this.querySelector('axa-icon[icon=menu]').addEventListener(
        'click',
        function (e) {
          // invert open state
    ...
    onOpenClose() {
      ...
      if (icon) icon.classList[this.isOpened ? 'add' : 'remove']('expanded');
    }

    setupOnTopMenuAction() {
      let header = this;
      this.querySelectorAll('slot[name=structure]>ul>li').forEach((elSubMenu) =>
        elSubMenu.addEventListener(
          'click',
          function (e) {
            //console.log(this)
            let elSubSubMenu = elSubMenu.querySelector('ul');
            // in there is no submenu, simply follow the link (if any)
            if (!elSubSubMenu) return true;
